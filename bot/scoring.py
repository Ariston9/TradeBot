...
def score_on_tf(df: pd.DataFrame, tf_name: str):
    ...
    if tf_name == "M1":
        ...
        if support is not None and price >= support and (price - support) <= level_eps:
            near_support = True
        if resistance is not None and price <= resistance and (resistance - price) <= level_eps:
            near_resistance = True

    smc = detect_smc_levels(df) if tf_name == "M1" else {
        "swing_high": None,
        "swing_low": None,
        "type": None,
        "strength": 0.0,
...
def calc_overall_probability(tf_results):
    ...
    high = float(m1.get("swing_high") or 0)
    low = float(m1.get("swing_low") or 0)
    close = float(m1.get("ema20") or 0)

    d5 = m5.get("direction") if m5 else "NONE"
    d15 = m15.get("direction") if m15 else "NONE"

    if d5 == direction:
        prob += 6.0
    elif d5 in ("BUY","SELL") and d5 != direction:
        prob -= 10.0

    if d15 == direction:
        prob += 8.0
    elif d15 in ("BUY","SELL") and d15 != direction:
        prob -= 12.0

    if direction in ("BUY","SELL"):
        conflict = False
        if d5 in ("BUY","SELL") and d5 != direction:
            conflict = True
        if d15 in ("BUY","SELL") and d15 != direction:
            conflict = True
        if not conflict:
            prob = max(prob, 70.0)
